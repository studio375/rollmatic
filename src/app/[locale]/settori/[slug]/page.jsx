...
export default async function Page({ params }) {
  ...
  return (
    <>
      <Breadcrumbs
        items={[
          { href: "settori", label: t("Settori") },
          { label: settore.title.rendered },
        ]}
        className="!relative !top-0 mt-10 mb-[5px]"
      />
      <section className="flex w-full boxed m:!px-4 min-[1920px]:!px-[3vw] max-s:!px-0 max-m:mt-[75px]">
        <Image
          className="max-xs:aspect-2/1.5 object-cover xs:h-[50vh] m:h-[75vh] w-full rounded-[5px]"
          src={img.source_url}
          width={img.media_details.width}
          height={img.media_details.height}
          alt={settore.title.rendered}
        />
      </section>
      <section className="w-full big-boxed mt-7 mb-10 relative max-s:mt-4">
        <div className="w-full flex items-start justify-between pb-3 border-b-[1px] border-b-[var(--color-primary)] max-l:flex-col max-l:gap-2">
          <BigText Tag="h1" className="classic-title">
            {settore.title.rendered}
          </BigText>
          <Paragraph className="w-[calc(100%/3*2.1)] max-l:w-full">
            {settore.acf.paragrafo}
          </Paragraph>
        </div>
      </section>
      ...
      {settore.acf.faq && settore.acf.faq.length > 0 && (
        <section className="big-boxed w-full relative pb-15 max-s:pb-10">
