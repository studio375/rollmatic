    ...
    <>
      ...
      <section className="w-full flex h-auto relative min-h-[50vh] testata-product pt-10 max-m:pt-12 max-m:pb-5 boxed xl:!px-15 flex items-center justify-between max-m:flex-col max-m:gap-4">
        <div className="relative flex flex-col items-start gap-[15px] w-40 max-m:w-full max-m:[&_>*]:w-full">
          ...
          <Paragraph className="text-[var(--color-foreground)]">
            {cat.acf.paragrafo}
          </Paragraph>
        </div>
        {imgTestata && (
          <Image
            className="w-full h-auto m:h-[60vh] s:object-contain  w-auto object-cover m:max-w-[calc(100%-400px)]"
            src={imgTestata.url}
      ...
      {cat.acf.foto_emotional && (
        <section className="px-[75px] max-xl:px-3 mb-10 mt-5">
          <VideoCarousel videoIds={null} images={cat.acf.foto_emotional} />
        </section>
      )}
