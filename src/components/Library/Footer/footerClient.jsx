...
export default function FooterClient({widgets}){
    ...
    var bottom = widgets.filter((widget) => widget.sidebar === "sidebar-bottom").map((widget) => {
        return (<div className={`footer-bottom px-2 ${!isContatti && 'mt-25 max-xl:mt-15 max-s:mt-10 max-xs:mt-5'} w-full`} key={widget.id}>{parse(widget.rendered)}</div>);
    });
    return <footer id="site-footer" className={`relative w-full bg-[var(--color-primary)] ${isContatti? 'pt-3' : 'pt-30 max-xl:pt-15 max-s:pt-10 max-xs:pt-6'} pb-2`}>
        {
