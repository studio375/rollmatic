...
export default function Faq({faq}){
    ...
    return <div ref={ref} className="flex flex-col items-start w-full">
        {
            faq.map((elem, index) => {
                return <div onClick={() => handleClickFaq(ref.current, index)} key={index} className="single-line w-full py-2 border-b-[1px] border-b-[var(--color-primary)] [&:first-child]:border-t-[1px] [&:first-child]:border-t-[var(--color-primary)] cursor-pointer transition-all duration-500 [&:hover:not(.open)]:py-[25px]">
                    <div className="domanda w-full flex items-center">
                        ...
                        <div className={`w-3 h-3 max-m:w-2 max-m:h-2 ml-auto relative transition-all duration-500 [.single-line:hover:not(.open)_&]:rotate-180`}>
